...
export interface TargetCursorProps {
  ...
  /**
   * When set, the custom cursor is only visible (and the default cursor only
   * hidden) while the pointer is inside an element matching this selector.
   * Outside those zones the normal system cursor is shown. Leave undefined to
   * make the custom cursor active across the whole page.
   */
  zoneSelector?: string;
}
...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // When scoped to zones, the default cursor is toggled dynamically in the
    // move handler; otherwise hide it globally for the whole page.
    if (hideDefaultCursor && !zoneSelector) {
    ...
    // Start hidden when zoned — it fades in once the pointer enters a zone.
    if (zoneSelector) {
    ...
    const moveHandler = (e: MouseEvent) => {
      if (zoneSelector) {
        const inside = !!(e.target as Element | null)?.closest(zoneSelector);
        if (inside !== insideZoneRef.current) {
          insideZoneRef.current = inside;
          if (inside) {
